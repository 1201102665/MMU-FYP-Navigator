...
// Fetch all resources and join with user and faculty data
...
$stmt->execute();
$resources = $stmt->fetchAll(PDO::FETCH_ASSOC); // Fetch all resources as an associative array

// Filter resources into approved and pending categories
...
// Handle form submissions for approving or deleting resources
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['approve'])) {
        $resource_id = $_POST['resource_id'];
        $stmt = $pdo->prepare("UPDATE resources SET pending_acceptance = 0 WHERE resource_id = ?");
        $stmt->execute([$resource_id]);
        $_SESSION['success'] = 'Resource approved successfully.';
    ...
    <div class="container">
        ...
        <?php if (count($pending_resources) > 0): ?>
            <table>
                ...
                <tbody>
                    <?php foreach ($pending_resources as $resource): // Loop through each pending resource ?>
                        <tr>
